...
def daily_buttons(tasks: List[Dict[str, Any]],
    ...
    Отмеченные приходят в `marked` — набор берётся из habit_marks
    за сегодняшний день. Сначала состояние жило в самой клавиатуре,
    но каждая новая команда «ежедневные» создавала новое сообщение
    с чистыми галочками, и ставить их приходилось заново.

    Кнопка — переключатель: нажал по ошибке, нажал снова и снял.
    Одностороннее действие заставляло бы ждать следующего дня.
    ...
    for t in tasks:
        ...
        rows.append([
            InlineKeyboardButton(f"{'✅' if done else '☐'} {label}",
                                 callback_data=f"hmark:{t['id']}"),
...
def build_daily(marked: frozenset = frozenset()
    ...
    Аргумент today не нужен: у ежедневных нет даты. А `marked` — это
    набор отмеченных сегодня, его читает вызывающая сторона из базы:
    вёрстка сама в базу за отметками не ходит, чтобы её можно было
    проверять без подключения.
    """
    daily = tasks_daily()
    ...
    lines += [daily_line(t) for t in daily]
    lines.append("")
    lines.append("<i>☐ отметить сделанное, нажать снова — снять · "
                 "🗑 убрать привычку насовсем</i>")
