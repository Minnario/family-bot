...
-- ОТМЕТКИ ПРИВЫЧЕК
-- ------------------------------------------------------------
-- Мотиватор, а не учёт выполнения. Нажал галочку — видишь галочку,
-- и всё: статус привычки остаётся pending, напоминания идут как
-- обычно, на просрочку это не влияет. Единственный потребитель —
-- список «ежедневные».
--
-- Сначала отметка жила в клавиатуре сообщения, но каждая новая команда
-- создавала новое сообщение с чистыми галочками.
--
-- Ключ из двух полей вместо id: отметка и есть пара «задача и день»,
-- второй раз за тот же день её не поставить. Повторное нажатие
-- удаляет строку, а не добавляет вторую.
...
-- Выборка всегда по дню. Индекс по дате нужен ещё и для чистки:
-- первичный ключ для неё не годится, он начинается с task_id.
CREATE INDEX idx_habit_marks_date ON habit_marks (mark_date);
...
--  WHERE mark_date < CURRENT_DATE - INTERVAL '60 days';
